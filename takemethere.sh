...
print_examples() {
    echo "Examples:"
    echo "  tmt 1                         # Change to directory on line 1"
    echo "  tmt my_alias                  # Change to directory associated with 'my_alias'"
    echo "  --"
    echo "  tmt -a /foo/bar               # Add path /foo/bar as the last entry"
    echo "  tmt -a bar /foo/bar           # Add lias 'bar' for path /foo/bar as the last entry"
    echo "  --"
    echo "  tmt -c foo bar /bar/          # Change 'foo' to 'bar' with path '/bar/'"
    echo "                                # Change 'my_alias' to 'new_alias' with path '/path/to/new/dir'"
    echo "  tmt -d my_alias               # Delete 'my_alias'"
...
print_help() {
    ...
    echo ""
    echo "Description: TakeMeThere simplifies navigation to frequently visited directories using user-defined aliases stored in a user defined file (defualt: ~/.takemethere). Quickly access directories by alias (tmt work) or sequential order (tmt 1, tmt 2, etc.). Easily manage, reorder, and edit aliases through the CLI or using your preferred text editor."
    echo ""
    print_options
    echo ""
    print_examples
